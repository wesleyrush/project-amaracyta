...
export default function Chat(){
  ...
  const { cid, user, balances, costs, setBalances } = useApp();
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!hasUserMsgRef.current) {
        if (useOpening) {
          // Fluxo com opening prompt: envia mensagem oculta e aguarda resposta da IA
          const alreadySent = openingSentKey && sessionStorage.getItem(openingSentKey) === '1';
          if (!alreadySent) {
            if (noBalance) {
              // Sem saldo: exibe welcome_message como fallback em vez do opening prompt
              const rawWelcome = s.module_welcome_message || `Olá, ${first}! Como posso ajudar hoje?`;
              const fallbackText = rawWelcome.replace(/\{first\}/g, first);
              setMsgs(prev => {
                const alreadyHas = prev.some(m => m.role === 'assistant' && m.content === fallbackText);
                if (alreadyHas) return prev;
                return [...prev, { role: 'assistant', content: fallbackText } as Message];
              });
              setBusy(false);
            } else {
              setMsgs(prev => {
